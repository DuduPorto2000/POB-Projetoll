...
public class Fachada {
	...
	public static List<Mensagem> buscarMensagens(String termo) throws  Exception{
		...
		if(retorno == null) {
			DAO.rollback();
			throw new Exception("n�o existe mensagem com este termo.");
		}
		DAO.commit();
		return retorno;
	}
...
//	}
	...
	public static List<Mensagem> listarMensagensUsuario() throws Exception{
		...
		if (getLogado() != null)
			...
			throw new Exception("O usuario n�o est� logado");
	}
	...
	public static void apagarMensagens(int... ids) throws  Exception{
		...
		for (int i : ids) {
			...
			if(!usuariologado.getMensagens().contains(m)) {
				DAO.rollback();
				throw new Exception("Mensagem n�o pertence ao usu�rio logado.");
			}
			if(m == null) {
				DAO.rollback();
				throw new Exception("Mensagem n�o encontrada.");
...
//	}
	...
	public static void ativarUsuario(String nome) throws  Exception{
		...
		if(getLogado().getNome() != "admin") {
			DAO.rollback();
			throw new Exception("O usu�rio deve ser administrador para executar essa a��o.");
		}
		if(!u.ativo()) {
			DAO.rollback();
...
//		criarMensagem(u.getNome() + " entrou no grupo");
	...
	public static void apagarUsuario(String nome) throws  Exception{
		...
		if(u.ativo() && getLogado().getNome() != "admin") {
			DAO.rollback();
			throw new Exception("Usu�rio tem que estar desativado.");
		}
